...
def render() -> None:
    ...
    st.markdown(
        ...
        "EOS Migration, Insights, and every Trend Analysis page for an AVS "
        "category) show **only** `is_from_avs = FALSE`.\n"
        "- **AVS → Azure Native** — its Migration Analytics dashboard, its Status "
        "report and its Trend Analysis pages — shows **only** "
        "`is_from_avs = TRUE`.\n\n"
    ...
    st.markdown(
        ...
        "record list only.\n"
        "- **Trend Analysis** re-cuts the *same* month-over-month trends the "
        "Migration Analytics dashboards draw, one trend per page across the "
        "migration categories: **Nomination Trends**, **ACR Trend**, **Nodes "
        "Deployed** (**Cores Migrated** for AVS → Azure Native) and **Migrations "
        "Completed**. Both routes run the same calculation — the dashboard shows "
        "one category's four trends, Trend Analysis shows one trend across the "
        "categories — so the numbers cannot drift apart.\n"

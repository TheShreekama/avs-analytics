...
def render() -> None:
    ...
    st.markdown(
        ...
        "`7 - Completed` **and none of whose waves is On Track** | Customers | "
        "Actual End Date |\n"
    ...
    st.markdown(
        ...
        "everything\". Those accounts are reported in full in **Accounts outside "
        "the reported pipeline**, a section of its own on every dashboard and in "
        "both export formats (in the HTML report it sits behind a checkbox and "
        "is hidden until ticked). It reports their count, the ACR they hold up, "
        "their state and WW Region distributions, the reason each carries and "
        "how many waves sit behind them.\n\n"
    ...
    st.markdown(
        ...
        "**Accounts outside the reported pipeline** sits at the end of each "
        "report, behind a **checkbox that starts unticked** — the section is "
        "hidden by a CSS rule on the checkbox's own state, so it is hidden from "
        "the first paint with no script having run, which is what makes it work "
        "in a file opened offline from a mail attachment. The data is always "
        "generated; only its visibility is deferred.\n\n"
        "Each report closes with a **Methodology & logic** section — the same "
        "rules this page states, rendered from the same source — so a circulated "
        "report carries its own definitions.\n\n"
        ...
        "behind the regional heatmap, the pipeline tables, and the account records "
